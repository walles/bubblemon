...
static uint64_t sampleMemoryPressure(char *preallocated, long preallocated_size) {
...
static void *measureMemoryPressureThread() {
  ...
  while (1) {
    uint64_t newMemoryPressure = sampleMemoryPressure(preallocated, preallocated_size);

    assert(0 == pthread_mutex_lock(&memoryPressureMutex));
    memoryPressure = newMemoryPressure;
